...
- **Homepage Content Management**: Hero Carousel slides, Promotion Grid cards, and Product YouTube Shorts/Videos are stored in the singleton `content` document schema. The backend initializes default seed content on startup/query, validates mutations via Zod schemas at `PUT /settings/content`, and serves active items (`active !== false`) ordered by `sortOrder` on `GET /storefront/settings`. Admin URL entries automatically extract YouTube Video IDs and fallback to YouTube HQ thumbnails when no custom image is supplied.
